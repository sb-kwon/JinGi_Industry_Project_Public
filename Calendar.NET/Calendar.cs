...
namespace Calendar.NET
{
    ...
    public class Calendar : UserControl
    {
        ...
        public Calendar()
        {
            ...
            _todayFont = new Font("궁서체", 10, FontStyle.Bold);
        ...
        private void CalendarMouseMove(object sender, MouseEventArgs e)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                    ...
                    (_calendarView == CalendarViews.Day && z.EventArea.Contains(e.X, e.Y + _scrollPanel.ScrollOffset) && z.Event.TooltipEnabled))
                {
                    ...
                    //Here the inner message of event.
                    _eventTip.EventToolTipText = z.Event.EventMessage;
                    if (z.Event.IgnoreTimeComponent == false)
                        _eventTip.EventToolTipText += "\n" + z.Event.Date.ToShortTimeString();

                    //Here we are added some control for overflow

                    int _OverflowMarginTop = 0, _OverflowMarginRight = 0;
                    if (_eventTip.Height > e.Y)
                    {
                        _OverflowMarginTop = _eventTip.Height - e.Y;
                    }
                    if (_eventTip.Width + e.X > this.Width)
                    {
                        _OverflowMarginRight = (e.X - _eventTip.Width) - e.X;
                    }

                    //And here we are adding the overflow margins for top and right.
                    _eventTip.Location = new Point(e.X + 5 + _OverflowMarginRight, e.Y - _eventTip.CalculateSize().Height + _OverflowMarginTop);
        ...
        private void RenderMonthCalendar(PaintEventArgs e)
        {
                    ...
                    if (rogueDays == 0 && counter <= DateTime.DaysInMonth(_calendarDate.Year, _calendarDate.Month))
                    {
                        ...
                        if (first == false)
                        {
                         ...
                         && counter == DateTime.Now.Day)
                            {
                                g.DrawString(
                                    _calendarDate.ToString("MMM") + "월 " + counter.ToString(CultureInfo.InvariantCulture) + "일",
            ...
            g.DrawString("Sat", _dayOfWeekFont, Brushes.Blue, xStart, yStart);
